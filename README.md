...
Docker Compose로 백엔드가 실행 중인 상태에서 루트 디렉터리에서 실행합니다.

```powershell
./scripts/verify-backend-lv1.ps1
./scripts/verify-backend-lv2.ps1
./scripts/verify-backend-lv3.ps1
```

각 스크립트는 실제 HTTP 요청으로 주요 API 흐름을 확인합니다.
...
이 서비스는 "책 만들기"를 먼저 앞세우기보다, 책으로 만들 수 있는 콘텐츠가 자연스럽게 쌓이는 흐름을 만들기 위해 기획했습니다.  
책 자체보다 사용자가 직접 고른 문장과 메모를 기록의 최소 단위로 보았고, 이 기록이 쌓이면 기간별 독서 기록책 주문으로 이어지도록 설계했습니다.

주문 데이터는 현재 DB를 다시 조회하지 않고, 주문 생성 시점에 저장한 `snapshotJson`을 기준으로 내보냅니다.  
이미 생성된 주문은 특정 시점의 기록을 확정한 결과이므로, 이후 책이나 인용문이 수정되더라도 export 결과가 바뀌지 않아야 한다고 판단했습니다.
